...
function generateRoundRobin(teams: string[]){ const n=teams.length, arr=[...teams], weeks: {home:string; away:string}[][]=[]; for(let r=0;r<n-1;r++){ const pairs: {home:string; away:string}[]=[]; for(let i=0;i<n/2;i++){ pairs.push({home:arr[i], away:arr[n-1-i]}); } weeks.push(pairs); arr.splice(1,0,arr.pop() as string); } return weeks; }
...
export default function VeggenteApp(){
  ...
  function ensureUserNode(email:string, name:string){ setPicks(prev=>{ const next={...prev}; if(!next[email]) next[email]={name, weeks:{}}; if(!next[email].weeks[week]) next[email].weeks[week]={}; return next; }); }
  function setPick(email:string, name:string, wk:number, matchNumber:number, value:'1'|'X'|'2'){ setPicks(prev=>{ const n={...prev}; if(!n[email]) n[email]={name, weeks:{}}; if(!n[email].weeks[wk]) n[email].weeks[wk]={}; n[email].weeks[wk][matchNumber]=value; return n; }); }
  function setResult(wk:number, matchNumber:number, hg:any, ag:any){ setResults(prev=>({ ...prev, [`${wk}_${matchNumber}`]: { hg, ag } })); }
  ...
  // scoring
  const weeklyScores = useMemo(()=>{ const out: Record<number, Record<string, {correct:number; perfect:boolean; points:number; name:string}>> = {}; const {pointsCorrect, bonusPerfectWeek, matchesPerWeek}=settings; Object.entries(picks).forEach(([email,u])=>{ Object.entries(u.weeks||{}).forEach(([wkStr, mmap])=>{ const wk=Number(wkStr); let correct=0; for(let m=1;m<=5;m++){ const pick=(mmap as any)[m]; const res=results[`${wk}_${m}`]; if(!res) continue; const sign=resultToSign(res.hg,res.ag); if(pick&&sign&&pick===sign) correct++; } const perfect= correct===matchesPerWeek; const points= correct*pointsCorrect + (perfect? bonusPerfectWeek: 0); if(!out[wk]) out[wk]={}; out[wk][email]={correct, perfect, points, name:u.name||email}; }); }); return out; }, [picks, results, settings]);
  const leaderboard = useMemo(()=>{ const acc: Record<string, {name:string; correct:number; perfects:number; points:number}> = {}; Object.values(weeklyScores).forEach((obj)=>{ Object.entries(obj).forEach(([email,s])=>{ if(!acc[email]) acc[email]={name:s.name, correct:0, perfects:0, points:0}; acc[email].correct+=s.correct; acc[email].points+=s.points; if(s.perfect) acc[email].perfects+=1; }); }); return Object.entries(acc).map(([email,v])=>({email,...v})).sort((a,b)=> b.points-a.points || b.correct-a.correct || b.perfects-a.perfects); }, [weeklyScores]);
  const myWeeklyData = useMemo(()=>{ if(!user.email) return []; const rows: {week:string; points:number}[]=[]; for(let w=1; w<=settings.weeks; w++){ const s=weeklyScores[w]?.[user.email as string]; rows.push({ week:`W${w}`, points: s? s.points: 0 }); } return rows; }, [weeklyScores, user, settings.weeks]);

  // tests
  function Tests(){ const [res, setRes]=useState<{name:string; pass:boolean}[]>([]); function run(){ const tests: {name:string; pass:boolean}[]=[]; tests.push({name:"resultToSign 2-1 => 1", pass: resultToSign(2,1)==="1"}); tests.push({name:"resultToSign 1-1 => X", pass: resultToSign(1,1)==="X"}); tests.push({name:"resultToSign 0-3 => 2", pass: resultToSign(0,3)==="2"}); const sched= toSchedule(defaultTeams,38); tests.push({name:"toSchedule length 38*5", pass: sched.length===190}); const w1=sched.filter(s=>s.week===1); const seen=new Set<string>(); let dup=false; for(const m of w1){ if(seen.has(m.home)||seen.has(m.away)){ dup=true; break;} seen.add(m.home); seen.add(m.away);} tests.push({name:"unique teams in week 1", pass: !dup}); setRes(tests);} return (
        ...
        {matchesThisWeek.map(m=>{ const key=`${m.week}_${m.matchNumber}`; const myPick=picks[user.email]?.weeks?.[week]?.[m.matchNumber]||""; const res=results[key]||{hg:"", ag:""}; const sign=resultToSign(res.hg,res.ag); const done=!!sign; const correct= done && myPick && myPick===sign; return (
          <Card key={key} className="shadow-sm">
            ...
            <CardContent className="flex items-center justify-between gap-3 flex-wrap">
              ...
              {adminMode && (
                <div className="flex items-center gap-2">
                  <Input type="number" inputMode="numeric" className="w-16" placeholder="HG" value={res.hg} onChange={(e)=> setResult(m.week, m.matchNumber, (e.target as HTMLInputElement).value, res.ag)}/>
                  <span className="text-zinc-500">-</span>
                  <Input type="number" inputMode="numeric" className="w-16" placeholder="AG" value={res.ag} onChange={(e)=> setResult(m.week, m.matchNumber, res.hg, (e.target as HTMLInputElement).value)}/>
                </div>
              )}
            </CardContent>
          </Card>
  ...
  function SettingsTab(){ const [teamInputs, setTeamInputs]=useState(teams.join("\n")); function regen(){ const list=teamInputs.split("\n").map(s=>s.trim()).filter(Boolean); if(list.length!==settings.numTeams){ toast(`Inserisci esattamente ${settings.numTeams} squadre`); return;} const newSched= toSchedule(list, settings.weeks); setTeams(list); setSchedule(newSched); toast("Calendario rigenerato"); }
    return (
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="border-0 shadow-md"><CardHeader><CardTitle className="flex items-center gap-2"><Settings as={undefined} /><SettingsIcon className="w-5 h-5"/> Parametri</CardTitle><CardDescription>Personalizza punteggi e regole base</CardDescription></CardHeader>
